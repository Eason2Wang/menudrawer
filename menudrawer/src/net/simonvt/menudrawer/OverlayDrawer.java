...
public class OverlayDrawer extends DraggableDrawer {
    ...
    private final static int showMenuSize = 80;
    public final static int MENU_WIDTH = 60;
    ...
    @Override
    protected void drawOverlay(Canvas canvas) {//画content阴影效果的
        ...
        switch (getPosition()) {
            ...
            case RIGHT:
                mMenuOverlay.setBounds(0, 0, width - dpToPx(showMenuSize) + offsetPixels, height);
                break;
...
//        mMenuOverlay.setAlpha((int) (MAX_MENU_OVERLAY_ALPHA * openRatio));
    ...
    public void openMenu(boolean animate) {
        ...
        switch (getPosition()) {
            ...
            case RIGHT:
            case BOTTOM:
                animateTo = -mMenuSize + dpToPx(showMenuSize);
                break;
        }
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        mContentContainer.layout(0, 0, width - dpToPx(MENU_WIDTH), height);
        ...
        if (USE_TRANSLATIONS) {
            switch (getPosition()) {
                ...
                case RIGHT:
                    mMenuContainer.layout(width - dpToPx(showMenuSize) - mMenuSize, 0, width - dpToPx(showMenuSize), height);
                    break;
    ...
    protected void onMoveEvent(float dx, float dy) {
        switch (getPosition()) {
            ...
            case RIGHT:
                setOffsetPixels(Math.max(Math.min(mOffsetPixels + dx, 0), -mMenuSize + dpToPx(showMenuSize)));
                break;
        ...
        switch (getPosition()) {
            ...
            case RIGHT: {
                ...
                if (mIsDragging) {
                    ...
                    final int initialVelocity = (int) getXVelocity(mVelocityTracker);
                    mLastMotionX = x;
                    animateOffsetTo(initialVelocity > 0 ? 0 : -mMenuSize + dpToPx(showMenuSize), initialVelocity, true);
